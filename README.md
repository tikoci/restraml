...
> [!WARNING]
> Ironically, the RouterOS schema uncovered a bug in the most recent version of Postman that does not allow importing a RAML schema. Postman is working on a fix. Any existing loaded schema still works. Postman version 12.0.12 is the last version that allows importing the `RAML` schema generated by `restraml`. 
